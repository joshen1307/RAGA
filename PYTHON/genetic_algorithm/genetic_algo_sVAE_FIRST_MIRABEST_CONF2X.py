...
pyro.clear_param_store()
    ...
    # setup the VAE
vae = VAE(x_dim=X_DIM, y_dim=Y_DIM, h_dim_enco_1=H_DIM_ENCO_1, h_dim_enco_2=H_DIM_ENCO_2, h_dim_deco_1=H_DIM_DECO_1, h_dim_deco_2=H_DIM_DECO_1, z_dim=2, use_cuda=USE_CUDA)
...
def image_sample_plotter(epoch):
    with torch.no_grad():
        ...
        save_image(sample1.view(100, 1, 100, 100), 'fr1_sample_3_z_space_' +str(epoch)+'.png',nrow=10)
    
        sample2 = vae.decoder([z_fr2.cuda(),labels_y2.cuda().float()])

        save_image(sample2.view(100, 1, 100, 100), 'fr2_sample_3_z_space_' +str(epoch)+'.png',nrow=10) 
